import React from 'react'
import Link from 'next/link'

export default function ServicesTwo() {
    return (
        <section className='relative w-full bg-new-light-500'>
            <div className='w-full relative'>
                <h2 className='text-[40px] text-center max-w-3xl mx-auto  font-bold text-new-blue-300 '
                    style={{ lineHeight: '1.2' }}>
                    Smart Investment Services for Your Financial <strong className='text-new-green-500'>Growth</strong>
                    <img src="/assets/svg/services-02.svg" alt="growth" className='inline ml-3 mb-5' />
                </h2>

                <div className='w-full mt-10 relative'>
                    <div className='w-full grid md:grid-cols-5 grid-cols-1 gap-3'>
                        <div className='col-span-3 w-full relative p-8 rounded-2xl bg-new-green-300 '>
                            <div className='w-full flex gap-3.5 relative max-[550px]:flex-col'>
                                <div className='w-[50%] max-md:w-full'>
                                    <h2 className='font-semibold text-new-blue-500 text-3xl'>Direct Stock Investment</h2>
                                    <p className='text-new-dark-300 mt-3 text-lg'>
                                        Buy and sell stocks with expert insights and real-time market support for informed decision-making.
                                    </p>
                                </div>
                                <div className='flex-auto'>
                                    <img src="/assets/svg/services-03.svg" alt=""
                                        className='md:ml-auto max-[550px]:mx-auto max-[550px]: mt-8' />
                                </div>
                            </div>
                        </div>

                        <div className='col-span-2 w-full relative p-8 rounded-2xl bg-new-green-300 '>
                            <div className='w-full flex flex-col gap-3.5 relative'>
                                <h2 className='font-semibold text-new-blue-500 text-3xl'>Mutual Funds Advisory</h2>
                                <p className='text-new-dark-300  text-lg'>
                                    Get personalized mutual fund recommendations that match your financial goals and risk profile.
                                </p>
                            </div>
                        </div>

                        <div className='col-span-2 w-full relative p-8 rounded-2xl bg-new-green-300 '>
                            <div className='w-full flex flex-col gap-3.5 relative'>
                                <h2 className='font-semibold text-new-blue-500 text-3xl'>Demat Account Setup</h2>
                                <p className='text-new-dark-300  text-lg'>
                                    Open a free Demat account quickly and securely to start your investment journey with ease.
                                </p>
                            </div>
                        </div>

                        <div className='col-span-3 w-full relative p-8 rounded-2xl bg-new-green-300 '>
                            <div className='w-full flex flex-row-reverse gap-3.5 relative max-[550px]:flex-col'>
                                <div className='w-[50%] max-md:w-full '>
                                    <h2 className='font-semibold text-new-blue-500 text-3xl'>Portfolio Management</h2>
                                    <p className='text-new-dark-300 mt-3 text-lg'>
                                        Let professionals manage your investments with a strategic approach focused on long-term growth.
                                    </p>
                                </div>
                                <div className='flex-auto'>
                                    <img src="/assets/svg/services-04.svg" alt="" className='md:mr-auto max-[550px]:mx-auto max-[550px]: mt-8' />
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    )
}
